...
"""
all_x = []
authors = [all_x.append(x['author']) for x in detail_info_table.find()]
# print(len(all_x))
all_temp = ""
all_dict = {}
for temp in all_x:
...
plt.axis('off')
plt.show()
wc.to_file("pics/author-cloud.png")
...
#plt.grid('on')
plt.title('本刊目作者发文量排名', fontproperties=font_msyh)
#plt.ylabel('篇目', fontproperties=font_msyh)
plt.yticks(x, former_10_keys, fontproperties=font_msyh)
plt.savefig('pics/author-bar.png', dpi=300, format='png', bbox_inches='tight', transparent=True)
plt.show()
"""


all_x = []
organizations = [all_x.append(x['organization']) for x in detail_info_table.find()]
# print(len(all_x))
all_temp = ""
all_dict = {}
for temp in all_x:
...
del all_dict['550025']
all_dict['贵州大学数学系'] += all_dict['贵州大学数学系 贵阳']
all_dict['贵州大学数学系'] += all_dict['贵州大学数学系 贵阳 550025']
del all_dict['贵州大学数学系 贵阳']
del all_dict['贵州大学数学系 贵阳 550025']
# print(all_temp)
# with open("keywords.txt", 'w') as temp_f:
# 	temp_f.write(all_temp.encode("gbk", 'ignore').decode("gbk"))
# keywords_stopwords = set([x.strip() for x in codecs.open(".txt", 'r', "utf-8").read().split('\n')])
...
plt.axis('off')
plt.show()
wc.to_file("pics/organization-cloud.png")
...
ax.barh(x, former_10_values)
#plt.grid('on')
plt.title('本刊目组织结构发文量排名', fontproperties=font_msyh)
#plt.ylabel('篇目', fontproperties=font_msyh)
plt.yticks(x, former_10_keys, fontproperties=font_msyh)
plt.savefig('pics/organization-bar.png', dpi=300, format='png', bbox_inches='tight', transparent=True)
plt.show()
